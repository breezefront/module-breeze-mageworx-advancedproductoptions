...
(function (window, document, undefined) {
    // Uses AMD or browser globals to create a plugin on either jQuery or Cash.
    (function (factory) {
        "use strict";

        if (typeof define === "function" && define.amd) {
            // AMD loader: ask for “jquery” (if someone installed jQuery via AMD)
            define(["jquery"], factory);
        } else {
            // No AMD—try to use window.jQuery, or else fall back to window.cash:
            var host = window.jQuery || window.cash;
            if (host && !host.fn.qtip) {
                factory(host);
            }
        }
    ...
    (function ($) {
        ...
        // ────────────────────────────────────────────────────────────────
        // Shim for “$.data” and “$.removeData” when using Cash (no jQuery)
        // ────────────────────────────────────────────────────────────────
        (function ($) {
            if (!$.data) {
                // Use a WeakMap to hold per-element data objects
                var __dataStore = new WeakMap();

                // $.data(elem, key, value) | $.data(elem, key) | $.data(elem) | $.data(elem, {k1:v1, k2:v2})
                $.data = function (elem, key, val) {
                    if (!elem || elem.nodeType !== 1) return;

                    // If no data object exists yet for this element, create one:
                    if (!__dataStore.has(elem)) {
                        __dataStore.set(elem, {});
                    }
                    var elemData = __dataStore.get(elem);

                    // Setter mode: $.data(elem, key, val)
                    if (typeof key === "string" && arguments.length === 3) {
                        elemData[key] = val;
                        return val;
                    }
                    // Bulk‐setter: $.data(elem, { foo:1, bar:2 })
                    ...
                    // Getter mode: $.data(elem, key)
                    if (typeof key === "string" && arguments.length === 2) {
                        return elemData[key];
                    }
                    // No key provided: return a shallow clone of all data
                    return Object.assign({}, elemData);
                };

                // $.removeData(elem, key) or $.removeData(elem)
                $.removeData = function (elem, key) {
                    if (!elem || elem.nodeType !== 1 || !__dataStore.has(elem))
                        return;
                    if (typeof key === "string") {
                        var current = __dataStore.get(elem);
                        delete current[key];
                        if (Object.keys(current).length === 0) {
                            __dataStore.delete(elem);
                        }
                    } else {
                        // No key: remove all data for this element
                        __dataStore.delete(elem);
                    }
                };
            }
        })(window.$ || window.cash || window.jQuery);
        // ────────────────────────────────────────────────────────────────
        ...
        PROTOTYPE._assignInitialEvents = function (event) {
            ...
            function hoverIntent(hoverEvent) {
                ...
                this.timers.show = delay.call(
                    ...
                    function () {
                        this.render(
                            typeof hoverEvent === "object" || options.show.ready
                        );
                    },
        ...
        $.each(
            {
                ...
                clone: function (keepData) {
                    ...
                    var elems = $.fn["clone" + replaceSuffix].apply(
                        this,
                        arguments
                    );
            ...
            update: function (corner, position) {
                ...
                // VML (IE Proprietary implementation)
                    ...
                    inner
                        .css({
                            coordsize:
                                newSize[0] + border + " " + newSize[1] + border,
                            antialias:
